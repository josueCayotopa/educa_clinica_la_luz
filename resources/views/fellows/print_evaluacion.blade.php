...
<div class="max-w-5xl mx-auto p-6 bg-white shadow rounded-lg">
  ...
  <div class="grid grid-cols-2 gap-4 mb-4">
    ...
    <div>
      ...
      <div><span class="font-semibold">Promedio (0–5):</span>
        {{ $currentAverage !== null ? number_format($currentAverage,2) : '—' }}</div>
    </div>
  </div>
  ...
  <h2 class="text-lg font-semibold mb-2">Curva de aprendizaje (promedio 0–5)</h2>
  <div id="curveWrapper" class="bg-white p-3 border rounded">
    ...
    <div class="text-sm text-gray-700 mt-2">
      ...
      <span class="align-middle">Deficientes (&lt;3.5)</span>,
      <span class="inline-block w-3 h-3 align-middle rounded ml-2" style="background:rgba(234,179,8,.35)"></span>
      <span class="align-middle">Medios (3.5–4.4)</span>,
      <span class="inline-block w-3 h-3 align-middle rounded ml-2" style="background:rgba(34,197,94,.35)"></span>
      <span class="align-middle">Buenos (≥4.5)</span>.
      Eje X: <strong>número de caso</strong> (experiencia acumulada). Eje Y: <strong>promedio</strong> (0–5).
    </div>
  </div>
...
const scores = @json($scores);  // Now 0-5 scale instead of 0-100
...
const bandsPlugin = {
  ...
  beforeDraw(chart) {
    ...
    ctx.fillRect(left, y0(3.5), right-left, y0(0)-y0(3.5));
    // Amarillo 3.5–4.5
    ctx.fillStyle = 'rgba(234, 179, 8, 0.12)';
    ctx.fillRect(left, y0(4.5), right-left, y0(3.5)-y0(4.5));
    // Verde 4.5–5
    ctx.fillStyle = 'rgba(34, 197, 94, 0.12)';
    ctx.fillRect(left, y0(5), right-left, y0(4.5)-y0(5));
    ctx.restore();
  }
};
